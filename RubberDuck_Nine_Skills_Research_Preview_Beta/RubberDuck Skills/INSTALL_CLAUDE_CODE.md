...
Install all Claude Code skills:

```bash
mkdir -p ~/.claude/skills
rsync -a ./claude-code/.claude/skills/ ~/.claude/skills/
```

If `rsync` is unavailable, use `cp -a ./claude-code/.claude/skills/. ~/.claude/skills/`.

Optional: add the enforcement block from `claude-code/CLAUDE.md_SNIPPET.md` to your Claude Code instructions file.
...
```bash
python3 verification/verify_installed_skills.py ~/.claude/skills --write verification/installed_claude_results.json
```

Optional per-skill debugging only, if the all-check verifier reports a failure:

```bash
python3 ~/.claude/skills/<skill-slug>/scripts/verify_skill_structure.py ~/.claude/skills/<skill-slug>
python3 ~/.claude/skills/<skill-slug>/scripts/smoke_test_skill.py ~/.claude/skills/<skill-slug>
```
